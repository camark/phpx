...
if(!class_exists('PHPHand_Action')) exit('model can only runs by a action');
/**
 * PHPHand model basic class
 * we recommend all applications' model extends
 * this class
 * so that we can extern the model in here
 */
class PHPHand_Model extends PHPHand{
	/**
	 * base action
	 */
	...
	function __construct($action=NULL,$data_table=''){
		parent::__construct();
		if(($action instanceof PHPHand_Action)){
	...
	function get_table_and_column(){
		...
		if($this->db_pre && strpos($table,'plugin_')!==0 && strpos($table,$this->config->get('db_pre'))!==0){
