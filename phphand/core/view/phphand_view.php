 ...
 *
 */
include PHPHAND_DIR.'/core/view/phphand_tag_parser.php';
//include PHPHAND_DIR.'/core/lang/phphand_lang.php';
//include PHPHAND_DIR.'/Core/Cache/PHPHand_Cache.php';

/**
 * phphand view class
 *
 * is also the phphand template class
 * and main method phphand use to make MVC mode
 */
class PHPHand_View extends PHPHand{
	...
	function display($tpl='',$layout='',$outputhtml='',$return_dir=false,$ignore_static=false){
		...
		$cache_folder=strtolower($this->env->get('class').'$'.$this->env->get('method'));
		...
		if(strpos($tpl,'../../../')===0)
		{
			$path=$this->env->get('data_dir').strtolower("/tpl/". base64_encode($layout_prefix . '$'.$tpl).'.php');
		}else{
			$path=$this->env->get('data_dir').strtolower("/tpl/" . base64_encode($layout_prefix . '$'.$style . '--' . $cache_folder . '--' .$this->_cache_pre . str_replace("/","--",$tpl) ) .".php");
		}
		...
		if(!file_exists($path) || $this->env->get('debug_mode')){
			...
			$this->source_manager->save($path,$content);
		}
	...
	function parse($tpl,$isFileName=true,$ignore_static=false){
		$tagParser = PHPHand_Tag_Parser::getInstance();
		...
		//$urlParser=PHPHand_URL_Parser::getInstance();
	...
	function initStaticVar(){
		if(!isset($this->_staticList['__PHPHAND__']))
			$this->_staticList['__PHPHAND__']=$this->env->get('phphand_url');
		if(!isset($this->_staticList['__WEB__']))
			$this->_staticList['__WEB__']=$this->env->get('app_url');
		if(!isset($this->_staticList['../public']))
			$this->_staticList['../public']=$this->env->get('app_url').'/'.$this->env->get('app').'/view/'.$this->config->get('style').'/public';;
		if(!isset($this->_staticList['../Public']))
			$this->_staticList['../Public']=$this->env->get('app_url').'/'.$this->env->get('app').'/view/'.$this->config->get('style').'/public';;
