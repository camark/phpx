<?php
/**
 * PHPHand_Tag_Parser parse the phphand template tag
 *
 * this class compatible with old version of phphand
 ...
 */
class PHPHand_Tag_Parser extends PHPHand{
	/**
	...
	static function getInstance(){
		 ...
		 * make it a PHPHand_View instance
		 * and return it
		 */
		if(!self::$_instance){
	...
	function parseTag($taglib,$tag,$attributeString,$innerHTML=''){
		...
		if(is_bool($temp) && !$temp){
			return 'tag '.$taglib.':'.$tag.' not exists';
		}
	...
	function getTagDefination($taglib,$tag){
		...
		/**
		if(file_exists($this->env->get('app_dir').'/taglib/'.$taglib.'/'.$tag) && is_dir($this->env->get('app_dir').'/taglib/'.$taglib.'/'.$tag)){
			$url=$this->env->get('app_dir').'/taglib/'.$taglib;
			$tagurl='__WEB__/'.$this->env->get('app').'/taglib/'.$taglib.'/'.$tag;
		}elseif(file_exists(PHPHAND_DIR.'/taglib/'.$taglib.'/'.$tag) && is_dir(PHPHAND_DIR.'/taglib/'.$taglib.'/'.$tag)){
			$url=PHPHAND_DIR.'/taglib/'.$taglib;
			$tagurl='__PHPHAND__/taglib/'.$taglib.'/'.$tag;
		}else{
			return false;
		}
		*/
