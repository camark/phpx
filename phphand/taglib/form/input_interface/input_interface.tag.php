<?php
$args_part_index = $param.part_index;//获得下标
$args_part_sub_index = $param.part_sub_index;//下一级前缀
if($args_part_index===NULL) $args_part_index=false;
...
$INPUT_INTERFACE=1;
?>
<link rel="stylesheet" type="text/css" href="https://{$_SERVER.HTTP_HOST}__TAG__/tag.css" />
<div class="max-input-form">
	{if sizeof($pages)>1}
	...
	<div class="tab-content">
		...
		<div class="form-page" id="tab{$step}" index="{$step}"{if $step>0} style="display:none;"{/if}>
			...
			<?php 
			if($args_part_index!==false && $pindex != $args_part_index){continue;} ?>
			...
			<div class="form-part{if isset($part.multi) && $part.multi} form-part-multi{/if}" index="{$pindex}" multi="{$part.multi}" double="{$part.double}"{if isset($part.hide) && $part.hide} style="display:none;"{/if}>
				{if $args_part_index===false && $part.title!='default' && strpos($part.title,'#')!==0}<header>{$part.title}</header>{/if}
				<?php
				if(isset($part.multi) && $part.multi && isset($row) && isset($row[$part.title]) && is_array($row[$part.title]))
				{
					...
					if($args_part_sub_index===false){
						...
						{
							if($args_part_sub_index !==false && @$args_part_sub_index !== $sub_data['__namefix__'])
							{
							   continue;	
							}
				...
				{/if}
				{if $args_part_index===false && isset($part.multi) && $part.multi}
				<footer>
			...
			<div class="btn-line">
				<div class="row">
					<div class="col-md-<?php if(@$part.double) echo 4;else echo 2;?> col-sm-<?php if(@$part.double) echo 4;else echo 2;?> space">&nbsp;</div>
					<div class="col-sm-10 col-xs-12 col-md-<?php if(@$part.double) echo 4;else echo 2;?>">
						<input type="<?php if($step<sizeof($pages)-1) echo 'button';else echo 'submit';?>" value="<?php if($step<sizeof($pages)-1) echo '下一步';else echo '提交';?>" page="{$step}" check="no" class="btn btn-primary btn-lg btn-block" />
					</div>
				</div>
			</div>
			{/if}
...
$(function(){
	...
	page.find('.form-part footer a').click(function()
	{
		...
		var url = '__PHP__?class={form:input_interface}&config='+config+'&page_index='+page_index+'&part_index='+part_index;
	...
	if(typeof page.attr('role')!='undefine' && page.attr('role')=='sheet-box-container')
	{
		...
		a.remove();
		sheet_box_head.find('a').each(function(i)
		{
			if($(this).find('span').size()==0)
			{
				$(this).html('<span>'+(i+1)+'</span>'+$(this).html());
			}else{
