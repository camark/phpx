   ...
   public static function getInstance(){
	...
	function checkBase($field,$config)
	{
	   if(isset($config['is_must']) && $config['is_must'] && (!isset($_POST[$field]) || !$_POST[$field]))
		...
		if(empty($config['pattern']) && isset($config['minlength']) && $config['minlength'] && 
		...
		if(empty($config['pattern']) && isset($config['maxlength']) && $config['maxlength'] && 
...
<<<<<<< .mine
		if(!$_POST[$field] && !$config['is_must']) return '';
...
>>>>>>> .r1171
		{
			switch($config['pattern'])
			{
				case 'int':
					...
					if(isset($config['minlength']) && ( $_POST[$field] < intval($config['minlength']) ) )
					{
						exit($config['showname'].'不能小于'.$config['minlength']);
					}
					if(isset($config['maxlength']) && ( $_POST[$field] > intval($config['maxlength']) ) )
					{
