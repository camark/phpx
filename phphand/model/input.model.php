 ...
 * 以及调用输入输出接口的一组常用方法
 */
class InputModel extends PHPHand_Model
{
	/****************************************************************
	...
	*/
	...
	function build($config_file,$field,$config)
	{
		...
		{
			$example_config = $this->data_helper->read(__ROOT__.'/data/example/' . substr($config['input'],8) .'.php','config');
	...
	*/
	function load_checker($input)
	{
		...
		$path = PHPHAND_DIR.'/taglib/input/'.$input.'/__check.php';
		if(!file_exists($path))
		{
