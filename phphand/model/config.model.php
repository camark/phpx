<?php
class ConfigModel extends PHPHand_Model{
	...
	function init($app='app',$ext=NULL){
		 ...
		 * copy it from phphand default config file
		 */
		if(isset($_SERVER['SCRIPT_FILENAME'])){
			$appdir=str_replace('\\','/',dirname($_SERVER['SCRIPT_FILENAME'])).'/'.$app;
							
			if(!file_exists($appdir.'/config.php')){
				copy(PHPHAND_DIR.'/Template/Config/Default.php',$appdir.'/config.php');
			}
		}
		...
		@$this->_config=array_merge($this->_config,array_change_key_case($config));
		unset($config);
	...
	function setDefaultConfig(){
		$this->readFromFile(PHPHAND_DIR.'/template/config/default.php');
	}
